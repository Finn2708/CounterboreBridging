...
def rotateVector(vect:adsk.core.Vector3D,axis,angle_degrees):
    ...
    if axis == 'z':
        rotation_matrix.setToRotation(angle_radians, adsk.core.Vector3D.create(0, 0, 1),origin)
    elif axis == 'y':
        rotation_matrix.setToRotation(angle_radians, adsk.core.Vector3D.create(0, 1, 0),origin)
    elif axis == 'x':
        rotation_matrix.setToRotation(angle_radians, adsk.core.Vector3D.create(1, 0, 0),origin)
    else:
...
def getExtendedIntersectionPoints(line : adsk.fusion.SketchLine,curveArray):
    ...
    intersectionsWithLineStart= [i for i in intersectionsWithLine if round(getAngleFromTwoPoints(i["point"],middle),0) == startDirection  ]
    intersectionsWithLineEnd= [i for i in intersectionsWithLine if round(getAngleFromTwoPoints(i["point"],middle),0) == endDirection  ]     
...
def profileHasLine (profile :adsk.fusion.Profile,line:adsk.core.Line3D):
    ...
    """
    for l in profile.profileLoops:
        for c in l.profileCurves:
